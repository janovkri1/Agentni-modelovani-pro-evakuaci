...
class Map:
    ...
    def move(self, agent, new_position, agent_type):
        ...
        if self.map[new_position[0]][new_position[1]] == 'F' and agent_type == 'L':
            self.push_away([old_position], new_position)
    ...
    """
    def push_away(self, from_position, to_position):
        if self.map[to_position[0]][to_position[1]] == 'F':
            ...
            while position_to_move == self.followers[
                ...
                if position_to_move is not None and self.map[position_to_move[0]][
                    ...
                    self.push_away(from_position, follower_to_push_away)
    ...
    def load_leaders(self, config_file):
        ...
        for line in file:
            ...
            if b < a:
                inf = False
            leader = LeadingAgent(self.params.leader_sight, i, inf,
    ...
    def initialize(self):
        ...
        a = int(input())
        self.open_map('maps/building_small1.map')
        self.load_exits('exits/building_small1_exits.txt')
        self.leaders = self.load_leaders('scenarios/class_1D.txt')
        for leader in self.leaders:
            self.place_agent(leader)

        self.followers = self.load_followers('scenarios/agent_numbers/class_F' + str(a) + '.txt')
    ...
    def run(self):
        ...
        while True:
            ...
            for leader in noninformed_leaders:
                noninf_solution.append(leader.find_path(self.map, [], self.followers))
            for follower in self.followers:
                if not follower.evacuated:
                    for exit in self.exits:
                        if distance_from_agent(follower.position, exit) <= 2:
                            ...
                            follower.position = follower.known_exit
                            break
            ...
            for index, agent_path in enumerate(solution):
                if len(agent_path) < 2 or distance_from_agent(informed_leaders[index].position, informed_leaders[index].exit) <= 2:
                    if not informed_leaders[index].evacuated:
                        ...
                        informed_leaders[index].position = informed_leaders[index].exit
            ...
            for index, agent_path in enumerate(noninf_solution):
                if len(agent_path) < 2 or distance_from_agent(noninformed_leaders[index].position, informed_leaders[index].exit) <= 2:
                    if not noninformed_leaders[index].evacuated:
                        ...
                        noninformed_leaders[index].position = noninformed_leaders[index].exit
