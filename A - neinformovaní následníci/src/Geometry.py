...
class Geometry:
    def __init__(self, agent1_pos, agent2_pos, map_list):
        ...
        self.rect_bottom_left = [0, 0]
        self.rect_bottom_right = [0, 0]
        self.rect_top_left = [0, 0]
        self.rect_top_right = [0, 0]
    ...
    def set_state(self):
        # 1. define a rectangle with two of the tips being the agents' cell's coordinates
    ...
    def define_rectangle(self):
        self.rect_bottom_right = [max(self.agent1[0], self.agent2[0]), max(self.agent1[1], self.agent2[1])]
        self.rect_top_left = [min(self.agent1[0], self.agent2[0]), min(self.agent1[1], self.agent2[1])]
        self.rect_top_right = [min(self.agent1[0], self.agent2[0]), max(self.agent1[1], self.agent2[1])]
        self.rect_bottom_left = [max(self.agent1[0], self.agent2[0]), min(self.agent1[1], self.agent2[1])]
    ...
    def df_line_general_equation(self):
        # A1(x1, y1) a A2(x2, y2)
        # a = y2 - y1
        a = self.agent1[1] - (self.agent2[1] + self.deviation[1])
        # b = x1 - x2
        b = (self.agent2[0] + self.deviation[0]) - self.agent1[0]
        # c = a*x1 + b*y1
        c = a * (self.agent1[0] + self.deviation[0]) + b * (
                self.agent1[1] + self.deviation[1])
    ...
    def sees_through_intersections(self):
        for square in self.intersection_points:
            if self.map_list[square[0]][square[1]] == '@':
                ...
                if self.distance_point_line([square[0] + self.deviation[0], square[1] + self.deviation[1]]) <= 0.15:
                    return False
        return True
