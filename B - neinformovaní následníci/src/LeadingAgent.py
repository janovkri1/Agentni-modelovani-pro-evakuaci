...
class LeadingAgent(Agent):
    def __init__(self, sight_length, agent_number, min_swarm_denominator, is_informed, conflict_distance):
    ...
    def create_path(self, predecessor_tab):
        ...
        if predecessor_tab[actual_x][actual_y].x_pos == 0 and predecessor_tab[actual_x][actual_y].y_pos == 0:
            print(self.position)
            return [self.position, self.position] # == pokud neni nalezena cesta do cile, agent se nepohne
    ...
    def find_path(self, state, all_constraints, followers, is_cbs):
        ...
        if not self.check_swarm(state, followers, is_cbs) and self.time_since_moved < 25:
            if is_cbs < 2:
                self.time_since_moved += 1
            return [self.position, self.position, self.position, self.position, self.position]

        if (not self.check_swarm(state, followers, is_cbs) and self.time_since_moved >= 25) or self.forced_move:
            if is_cbs < 2:
